...
class ConfigurationClient implements Callback {
    ...
    private static final String MAPBOX_CONFIG_SYNC_KEY_TIMESTAMP = "dingiConfigSyncTimestamp";
    ...
    private final String dingiMapAccessToken;
    private final OkHttpClient client;
    private final List<ConfigurationChangeHandler> handlers;

    ConfigurationClient(Context context, String userAgent, String dingiMapAccessToken, OkHttpClient client) {
    ...
    void update() {
        HttpUrl requestUrl = generateRequestUrl(context,dingiMapAccessToken);
    ...
    private static HttpUrl generateRequestUrl(Context context,  String dingiMapAccessToken) {

        return new HttpUrl.Builder().scheme(HTTPS_SCHEME)
                .host(determineConfigEndpoint(context))
                .addPathSegment(EVENT_CONFIG_SEGMENT)
                .addQueryParameter(ACCESS_TOKEN_QUERY_PARAMETER, dingiMapAccessToken)
                .build();
